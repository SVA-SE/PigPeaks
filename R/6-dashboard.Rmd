...
if("time.to.reservice" %in% names(indicators.data)) {
  ...
  i = which(names(indicators.data)=="time.to.reservice")
    ...
    cat("  \n###", indicators.labels[i],", alarms per year","  \n")
  
plot_ly(alpha=0.8)%>%
  add_heatmap(
    type = "heatmap",
    y=count.ewma.alarms(df.indicator = indicators.continuous.to.weekly[[i]],
                        years.to.see = 3)[[2]],
    z = count.ewma.alarms(df.indicator = indicators.continuous.to.weekly[[i]],
                          years.to.see=3)[[1]],
    x=c(1:52),
    colors = colorRamp(c("purple", "lightblue", "red")),
    colorbar=list(tickvals = c(-3:3), len = 0.7),
    zauto = FALSE, zmin = -3, zmax = 3,
    ygap=7,
    showscale=TRUE) %>% 
...
if("time.to.reservice" %in% names(indicators.data)) {
  ...
  TS.barplot.pg.signals.continuous(df.indicator = indicators.continuous.to.weekly[[i]],
                                   target = NULL,
                                   target.unit = NULL, 
                                   ylabel = "average time (in days)",
                                   use.minimum.y = "min"
...
plot_ly(alpha=0.8)%>%
  add_heatmap(
    ...
    z = count.ewma.alarms(df.indicator = indicators.continuous.to.weekly[[i]],
                          years.to.see=3)[[1]],
    x=c(1:52),
    colors = colorRamp(c("lightblue", "red")),
    colorbar=list(tickvals = c(0:3), len = 0.7),
    zauto = FALSE, zmin = 0, zmax = 3,
    ygap=7,
    showscale=TRUE) %>% 
  layout(xaxis = list(dtick = 4),
         yaxis = list(dtick = 1))
...
plot_ly(alpha=0.8)%>%
  add_heatmap(
    ...
    z = count.ewma.alarms(df.indicator = indicators.continuous.to.weekly[[i]],
                          years.to.see=3)[[1]],
    x=c(1:52),
    colors = colorRamp(c("purple", "lightblue")),
    colorbar=list(tickvals = c(-3:0), len = 0.7),
    zauto = FALSE, zmin = -3, zmax = 0,
    ygap=7,
    showscale=TRUE) %>% 
  layout(xaxis = list(dtick = 4),
         yaxis = list(dtick = 1))
